...
@test "update of foo which is within 10 seconds" {
    ...
    run -0 executionMarker --timestamp "$NOW" --group "$BATS_TEST_NAME" --update "foo" --context "$CONTEXT" --newer 10
    assert_output ''
...
@test "update of fox which is not within 10 seconds" {
    ...
    run -1 executionMarker --timestamp "$NOW" --group "$BATS_TEST_NAME" --update "fox" --context "$CONTEXT" --newer 10s
    assert_output ''
...
@test "update of foo which is not without 10 seconds" {
    ...
    run -1 executionMarker --timestamp "$NOW" --group "$BATS_TEST_NAME" --update "foo" --context "$CONTEXT" --older 10
    assert_output ''
}
...
@test "update of fox which is without 10 seconds" {
    ...
    run -0 executionMarker --timestamp "$NOW" --group "$BATS_TEST_NAME" --update "fox" --context "$CONTEXT" --older 10s
    assert_output ''
}
...
@test "update of fox which is not within every 90 seconds returns the previous timestamp" {
    ...
    run -1 executionMarker --timestamp "$NOW" --group "$BATS_TEST_NAME" --update "fox" --context "$CONTEXT" --newer 90s --get-timestamp
