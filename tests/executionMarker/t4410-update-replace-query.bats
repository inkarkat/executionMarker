...
@test "update of an existing key overwrites that row with updated timestamp and context and returns the previous context" {
    ...
    run -0 executionMarker --timestamp "$NOW" --group "$BATS_TEST_NAME" --update "fox" --context "$CONTEXT" --get-context
...
@test "update of an existing key clears a previous context and returns the previous context" {
    ...
    run -0 executionMarker --timestamp "$NOW" --group "$BATS_TEST_NAME" --update "fox" --get-context
    assert_output 'Two minutes earlier than foo.'
...
@test "update of an existing key and --keep-context keeps a previous context and returns the previous context" {
    ...
    run -0 executionMarker --timestamp "$NOW" --group "$BATS_TEST_NAME" --update "fox" --keep-context --get-context
    assert_output 'Two minutes earlier than foo.'
...
@test "update of an existing key overwrites that row with updated timestamp and context and returns the previous timestamp" {
    ...
    run -0 executionMarker --timestamp "$NOW" --group "$BATS_TEST_NAME" --update "fox" --context "$CONTEXT" --get-timestamp
...
@test "update of an existing key overwrites that row with updated timestamp and context and returns the previous context and timestamp" {
    ...
    run -0 executionMarker --timestamp "$NOW" --group "$BATS_TEST_NAME" --update "fox" --context "$CONTEXT" --get-context --get-timestamp
    assert_output - <<'EOF'
Two minutes earlier than foo.
1557046597
EOF
}
