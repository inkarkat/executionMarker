...
@test "update of an existing key overwrites that row and returns the previous formatted time difference" {
    ...
    run -0 executionMarker --timestamp "$NOW" --group "$BATS_TEST_NAME" --update "fox" --context "$CONTEXT" --get-diff --output smallest-unit
...
@test "update of an existing key overwrites that row and returns the previous formatted time and difference" {
    ...
    run -0 executionMarker --timestamp "$NOW" --group "$BATS_TEST_NAME" --update "fox" --context "$CONTEXT" --get-time '%F %T' --get-diff --output smallest-unit
    assert_output - <<'EOF'
2019-05-05 08:56:37
133 seconds ago
EOF
